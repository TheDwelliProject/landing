...
function jwks() {
  if (!remoteJwks) {
    try {
      remoteJwks = createRemoteJWKSet(new URL(requiredEnv("AUTH_JWKS_URL")));
    } catch (cause) {
      if (cause instanceof JwtVerifierUnavailableError) throw cause;
      throw new JwtVerifierUnavailableError("invalid AUTH_JWKS_URL", { cause });
    }
  }
  return remoteJwks;
}
...
export async function verifyAccessJwt(token: string): Promise<AccessClaims> {
  ...
  try {
    ...
    if (isVerifierInfrastructureError(cause)) {
      throw new JwtVerifierUnavailableError(
        cause instanceof Error ? cause.message : "verification unavailable",
        { cause },
      );
    }
